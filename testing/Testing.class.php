 ...
 * Youds Framework testing environment
 * 
...
class Testing
{
	...
	//	$suites = include Cache::checkConfig(Config::get('core.testing_dir') . '/config/suites.xml');
		...
		if(version_compare(\PHPUnit\Runner\Version::id(), '3.6', '<')) {
			// PHP_CodeCoverage_Filter is a singleton
			$runner = new \PHPUnit\TextUI\TestRunner();
		} else {
			// PHP_CodeCoverage_Filter instance must be passed to the test runner
			$runner = new \PHPUnit\TextUI\TestRunner(null, self::$codeCoverageFilter);
		}
	 ...
	 * @deprecated 0.1 Use PhpUnitCli
	 */
	public static function getExitStatus(\PHPUnit\Framework\TestResult $result)
	{
	...
	public static function processCommandlineOptions()
	{
		...
		try {
			$options = \PHPUnit\Util\Getopt::getopt(
				$_SERVER['argv'],
				'd:',
				$longOptions
			);
		} catch(Exceptions\Runtime $e) {
			\PHPUnit\TextUI\TestRunner::showError($e->getMessage());
		}
