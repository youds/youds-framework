...
xmlns:framework_annotations="http://framework.youds.com/xml/config/global/annotations"
xmlns:framework_envelope="http://framework.youds.com/xml/config/global/envelope"
xmlns:framework_routing="http://framework.youds.com/xml/config/parts/routing"
...
>
	...
	<xsl:variable name="targetNamespace">
		<xsl:choose>
			...
			<xsl:when test="/framework_envelope:configurations/framework_envelope:configuration[@framework_annotations:matched and @targetNamespace]/@targetNamespace">
				<!-- take that one -->
				<xsl:value-of select="/framework_envelope:configurations/framework_envelope:configuration[@framework_annotations:matched and @targetNamespace][1]/@targetNamespace" />
			</xsl:when>
			<xsl:otherwise>
				<!-- there is none, take the one from the root element -->
				<xsl:value-of select="/framework_envelope:configurations/@targetNamespace" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="tns">
		<xsl:choose>
			...
			<xsl:when test="/framework_envelope:configurations/framework_envelope:configuration[@framework_annotations:matched and @targetNamespace and namespace::*[.=../@targetNamespace]]">
				<xsl:value-of select="name(/framework_envelope:configurations/framework_envelope:configuration[@framework_annotations:matched and @targetNamespace and namespace::*[.=../@targetNamespace]][1]/namespace::*[.=../@targetNamespace])" />
			</xsl:when>
			<!-- otherwise, take the namespace prefix from <configurations>, assuming that the targetNamespace is defined there, too -->
			<xsl:otherwise>
				<xsl:value-of select="name(/framework_envelope:configurations/namespace::*[.=../@targetNamespace])" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:template match="/framework_envelope:configurations">
		<wsdl:definitions name="Dummy">
			<!-- copy namespace nodes -->
			<xsl:copy-of select="namespace::*"/>
			<!-- copy per-context namespace nodes -->
			<xsl:copy-of select="framework_envelope:configuration[@framework_annotations:matched]/namespace::*"/>
			...
			<wsdl:types>
				...
				<xsl:copy-of select="framework_envelope:configuration[@framework_annotations:matched]/wsdl:types/@*" />
				<xsl:copy-of select="wsdl:types/node()" />
				<xsl:copy-of select="framework_envelope:configuration[@framework_annotations:matched]/wsdl:types/node()" />
			</wsdl:types>
			<!-- copy message defs -->
			<xsl:copy-of select="wsdl:message" />
			<!-- copy per-context message defs -->
			<xsl:copy-of select="framework_envelope:configuration[@framework_annotations:matched]/wsdl:message" />
			<!-- all the rest -->
			<xsl:apply-templates select="framework_envelope:configuration[.//framework_routing:route//wsdl:* | .//framework_routing:route//soap:*]" />
		</wsdl:definitions>
	</xsl:template>
	<xsl:template match="framework_envelope:configuration[@framework_annotations:matched]">
		<wsdl:portType name="DummyPortType">
			<xsl:apply-templates select=".//framework_routing:route" mode="port" />
		</wsdl:portType>
		<binding name="DummyBinding" type="tns:DummyPortType">
			<soap:binding style="rpc" transport="http://schemas.xmlsoap.org/soap/http"/>
			<xsl:apply-templates select=".//framework_routing:route" mode="binding" />
		</binding>
		...
		<xsl:apply-templates select=".//framework_routing:route" mode="messages" />
	</xsl:template>
	<xsl:template match="framework_routing:route" mode="port">
	...
	<xsl:template match="framework_routing:route" mode="messages">
		<xsl:variable name="name" select="translate(@pattern, '^$', '')" />
	...
	<xsl:template match="framework_routing:route" mode="binding">
		<xsl:variable name="name" select="translate(@pattern, '^$', '')" />
		<wsdl:operation name="{$name}">
