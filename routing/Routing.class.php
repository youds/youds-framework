...
/**
 *  allows you to centralize your entry point urls in your web
 * application.
 *
...
abstract class Routing extends ParameterHolder
{
	...
	/**
	 * @var        array An array of ArraySource.
	 */
	...
	{
		// get route
		$routeName = $this->getContext()->getRequest()->getAttribute('matched_routes', 'org.framework.routing');

		// return name
		return !empty($routeName) ? $routeName[count($routeName) - 1] : null;
	}
	 ...
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @author	   Craig Fairhurst <craigfairhurst@me.com>
	 */
	protected function loadConfig()
	{
		...
		if($this->isEnabled() || is_readable($cfg)) {
			$arr = array_merge($arr, unserialize(file_get_contents(Cache::checkConfig($cfg, $this->context->getName()))));	
		}
		$cfg = Config::get('core.config_dir') . '/routing.xml';
		if($this->isEnabled() || is_readable($cfg)) {
			$arr = array_merge($arr, unserialize(file_get_contents(Cache::checkConfig($cfg, $this->context->getName()))));
			$this->importRoutes($arr);
		}
		$cfg = Config::get('core.src_dir') . '/defaults/config/routing.xml';
		if($this->isEnabled() || is_readable($cfg)) {
			$arr = array_merge($arr, unserialize(file_get_contents(Cache::checkConfig($cfg, $this->context->getName()))));
			$this->importRoutes($arr);
		}
	...
	{
		$this->sources['_ENV'] = new Routing\ArraySource($_ENV);

		$this->sources['_SERVER'] = new Routing\ArraySource($_SERVER);

		if(Config::get('core.use_security')) {
			$this->sources['user'] = new Routing\UserSource($this->context->getUser());
		}
	}
	...
	public function addRoute($route, array $options = array(), $parent = null)
	{
		...
		if(isset($options['name']) && isset($this->routes[$options['name']])) {
			...
			if($parent !== null && $defaultOpts['parent'] != $parent) {
				throw new Exception('You are trying to overwrite a route but are not staying in the same hierarchy');
			}
			...
			$defaultOpts = array('name' => Toolkit::uniqid(), 'stop' => true, 'output_type' => null, 'content' => null, 'chain' => null, 'parameters' => array(), 'ignores' => array(), 'defaults' => array(), 'childs' => array(), 'callbacks' => array(), 'imply' => false, 'cut' => null, 'source' => null, 'method' => null, 'constraint' => array(), 'locale' => null, 'pattern_parameters' => array(), 'optional_parameters' => array(), 'parent' => $parent, 'reverseStr' => '', 'nostops' => array(), 'anchor' => self::ANCHOR_NONE);
		}
		...
		foreach($this->routes as $name => $route) {
			...
			if($route['opt']['parent'] == $routeName && !in_array($name, $options['childs'])) {
				throw new Exception('The route ' . $routeName . ' specifies a child route with the same name');
			}
		}
        ...
        $route['opt']['stop'] = false;
        if (str_ends_with($routeName, '.hook'))
            $route['opt']['stop'] = false;
		...
		/**
		 * executeHook support for all supported URLs
		 */
        if (str_ends_with($routeName, '.hook') === false):

            // regexp match string
            $_route = $route;

            // set hook
            $_route['opt']['stop'] = false;
            $_route['opt']['reverseStr'] = '/hook';
            $_route['opt']['output_type'] = 'hook';
            $_route['opt']['name'] = $routeName . '.hook';
            $_route['opt']['parent'] = $routeName;

            $this->addRoute('/hook', $_route['opt'], $routeName);
	...
	public function getAffectedRoutes($route, &$isNullRoute = false)
	{
		...
		if($route === null) {
			$includedRoutes = array_reverse($this->getContext()->getRequest()->getAttribute('matched_routes', 'org.framework.routing', array()));
			$isNullRoute = true;
	...
	protected function resolveGenOptions($input = array())
	{
			...
			foreach($input as $key => $value) {
				if(is_numeric($key)) {
					...
					if(isset($this->genOptionsPresets[$value])) {
						...
						throw new Exception('Undefined  gen() options preset "' . $value . '"');
					}
				} else {
		...
		throw new Exception('Unexpected type "' . gettype($input) . '" used as  gen() option preset identifier');
	}
		...
		foreach($routeNames as $routeName) {
			...
			if(count($r['opt']['callbacks']) > 0) {
				...
				foreach($r['callback_instances'] as $callbackInstance) {
					...
					if($callbackInstance instanceof ILegacyCallback) {
						...
						foreach($paramsCopy as &$param) {
							if($param instanceof IValue) {
								$param = $param->getValue();
							}
						}
						foreach($defaultsCopy as &$default) {
							if($default instanceof IValue) {
								$default = array(
						...
						// do *not* use this instead, it will segfault in PHP < 0.1:
						// $diff = array_udiff_assoc($paramsCopy, $changedParamsCopy, array($this, 'onGenerateParamDiffCallback'));
						// likely caused by http://bugs.php.net/bug.php?id=42838 / http://cvs.php.net/layoutvc.cgi/php-src/ext/standard/array.c?r1=0.1.0.1&r2=0.1.0.1
					} else {
						...
						foreach($diffKeys as $key) {
							...
							if($value !== null && !($value instanceof IValue)) {
								...
								if(isset($myDefaults[$key])) {
									if($myDefaults[$key] instanceof IValue) {
			...
			if(!(isset($finalParams[$name]) || array_key_exists($name, $finalParams))) {
				if($param === null && isset($optionalParams[$name])) {
					...
					if(isset($defaultParams[$name])) {
						if($param === null || ($param instanceof IValue && $param->getValue() === null)) {
	 ...
	 * 
	 * @param      mixed An IValue object or a string
	 * 
	...
	/**
	 * Converts all members of an array to IValues.
	 * 
	 * @param      array The parameters
	 * 
	 * @return     array An array containing all parameters as IValues
	 * 
	 * @author     Dominik del Bondio <dominik.del.bondio@bitextender.com>
	 */
	protected function convertParametersToValues(array $parameters)
	{
		if(count($parameters)) {
	...
	/**
	 * Generate a formatted Youds Framework URL.
	 *
	...
	public function gen($route, array $params = array(), $options = array())
	{
		...
		$params = $this->convertParametersToValues($params);
		...
		foreach($extras as &$extra) {
			$extra = ($extra instanceof IValue) ? $extra->getValue() : $extra;
		}
		...
		$uri = str_replace('//', '/', $prefix . str_replace($from, $to, $assembledInformation['uri'])); // don't allow double slashes
		return array($uri, $params, $options, $extras, $isNullRoute);
	}
	 ...
	 *          	or an Response if a callback returned one.
	 *
	 * @author	Dominik del Bondio <ddb@bitxtender.com>
	 * @author	Craig Fairhurst <craig.fairhurst@youds.com>
	 */
	public function execute()
	{
		...
		$umap = $rq->getParameter('use_content_chain_parameters');
		...
		// get matches
		$pattern = '/--([\w\-._]+)((=?\"([\w.\-_\s:\/]+)\")|(=?([\w@\/:._]+))?)/';
		preg_match_all($pattern, $input, $matches);

		for ($a = 0;$a < count($matches[6]);$a++):
			if (isset($matches[6][$a])):
				$match = trim($matches[6][$a]);
				$vars = array_merge($vars, array(
					$matches[1][$a] => strlen($match) > 0?$match:true
				));
			endif;
		endfor;
		$input = trim(str_replace('=', '', str_replace('  ', ' ', preg_replace($pattern, '', $input))));
		...
		do {
			...
			foreach($routes as $key) {
				...
				if(is_array($opts['constraint']) && (count($opts['constraint']) == 0 || (!empty($opts['constraint']) && in_array($requestMethod, $opts['constraint'])))) {
                    ...
                    if($this->parseInput($route, $input, $match)) {
						...
						if($opts['content']) {
							$content = Toolkit::expandVariables($opts['content'], $matchvals);
							$container->setContentName($content);
							if($umap) {
								$vars[$ma] = $content;
							}
						}
						...
						if($opts['chain']) {
							$chain = Toolkit::expandVariables($opts['chain'], $matchvals);
							$container->setChainName($chain);
							if($umap) {
								$vars[$aa] = $chain;
							}
						}
						...
						if(count($opts['callbacks']) > 0) {
							...
							foreach($route['callback_instances'] as $callbackInstance) {
								...
								if($callbackSuccess) {
									...
									$oldModule = $container->getContentName();
									$oldAction = $container->getChainName();
								...
								// to evaluate the contents of the attribute in question (e.g. content="${zomg}")
								if($opts['content'] && $oldModule == $container->getContentName() && (!$umap || !array_key_exists($ma, $vars) || $oldModule == $vars[$ma])) {
									$content = Toolkit::expandVariables($opts['content'], $expandVars);
									$container->setContentName($content);
									if($umap) {
										$vars[$ma] = $content;
									}
								}
								if($opts['chain'] && $oldAction == $container->getChainName() && (!$umap || !array_key_exists($aa, $vars) || $oldAction == $vars[$aa])) {
									$chain = Toolkit::expandVariables($opts['chain'], $expandVars);
									$container->setChainName($chain);
									if($umap) {
										$vars[$aa] = $chain;
									}
								...
								// one last thing we need to do: see if one of the callbacks modified the 'chain' or 'content' vars inside $vars if $umap is on
								...
								if($umap && $oldModule == $container->getContentName() && array_key_exists($ma, $vars) && $vars[$ma] != $oldModule) {
									$container->setContentName($vars[$ma]);
								}
								if($umap && $oldAction == $container->getChainName() && array_key_exists($aa, $vars) && $vars[$aa] != $oldAction) {
									$container->setChainName($vars[$aa]);
								}
							}
							if(!$callbackSuccess) {
		...
		if($container->getContentName() === null || $container->getChainName() === null) {
			
			// no route which supplied the required parameters matched, use 404 chain
			$container->setContentName(Config::get('chains.error_404_content'));
			$container->setChainName(Config::get('chains.error_404_chain'));
			
			if($umap) {
				$rd->setParameters(array(
					$ma => $container->getContentName(),
					$aa => $container->getChainName(),
				));
			}
		}
				
		// set the list of matched route names as a request attribute
		$rq->setAttribute('matched_routes', $matchedRoutes, 'org.framework.routing');
	...
	protected function parseInput(array $route, $input, &$matches)
	{
		...
		if($route['opt']['source'] !== null) {
			...
			if($count > 0 && isset($this->sources[$partArray[0]])) {
				...
				if($count > 1) {
					...
					} elseif($input instanceof ISource) {
		...
		// grab from normal rxp
		$retVal = preg_match($route['rxp'], $input, $initialMatch, PREG_OFFSET_CAPTURE);
        //dump($route['rxp'], $input, $initialMatch);

		// check for params
		$retValSections = 0;
		$matchesSectionsStitched = [];
		if (isset($route['rxpSections']) && is_array($route['rxpSections']) && count($route['rxpSections']) > 0):
			for ($i = 0;$i < count($route['rxpSections']);$i++):
				//preg_match($matches[0][$i], $matches[0][$i], $match, PREG_OFFSET_CAPTURE);
				$retValSections = preg_match('/' . $route['rxpSections'][$i] . '/', $input, $matchesSections, PREG_OFFSET_CAPTURE);
				$matchesSectionsStitched = array_merge($matchesSectionsStitched, $matchesSections);
			endfor;
		endif;
		
		// pass back
		if (isset($matchesSectionsStitched) && is_array($initialMatch) && is_array($matchesSectionsStitched) && count($initialMatch) > 0 && count($matchesSectionsStitched) > 0)
			$matches = array_merge($initialMatch, $matchesSectionsStitched);
		else
			$matches = $initialMatch;
				
		return $retVal > 0 || $retValSections > 0?1:0;
	}
			...
			if($state == 'start') {
				...
				if($c == '{') {
					...
					if($parenthesisCount > 0) {
						...
						if($parenthesisCount < 0) {
							throw new Exception('The pattern ' . $str . ' contains an unbalanced set of parentheses!');
						}
						...
						if(!$hasBrackets) {
							...
							if($bracketCount != 0) {
								throw new Exception('The pattern ' . $str . ' contains an unbalanced set of brackets!');
							}
							$rxPostfix = $tmpStr;
						}
						...
						if(!$rxName) {
							...
							if(!isset($vars[$rxName])) {
								if($rxPrefix && strpbrk($rxPrefix, $rxChars) !== false) {
									$rxPrefix = null;
								}
								if($rxInner && strpbrk($rxInner, $rxChars) !== false) {
									$rxInner = null;
								}
								if($rxPostfix && strpbrk($rxPostfix, $rxChars) !== false) {
									$rxPostfix = null;
								}
	...
	/**
	 * Creates and initializes a new IValue.
	 * 
	...
	{
		$value = new Routing\Value($value, $valueNeedsEncoding);
		$value->initialize($this->context);
		return $value;
	}
	
	/**
	 * Return matched route as array with values:
	 *  - route name (string)
	 *  - route url (string)
	...
	public function getMatchedRoute()
	{
		...
		return $rq->getAttribute('matched_routes', 'org.framework.routing');
	}
}
