...
/**
 * Web sets the prefix and input with some magic from the request
 * uri and path_info
 *
 ...
 * @version    $Id$
 */
class Web extends Routing implements IRouting
{
	/**
			...
			if($this->isEnabled()) {
				...
				foreach($routes as $route) {
					if(isset($this->routes[$route])) {
						...
						if($r['opt']['content']) {
							$myDefaults[$ma] = $r['opt']['content'];
						}
						if($r['opt']['chain']) {
							$myDefaults[$aa] = $r['opt']['chain'];
						}
