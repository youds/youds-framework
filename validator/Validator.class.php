...
abstract class Validator extends ParameterHolder
{
	...
	/**
	 * @var        DataHolder The parameters which should be validated
	...
	public function initialize(Context $context, array $parameters = array(), array $arguments = array(), array $errors = array())
	{
		...
		if(!isset($parameters['source'])) {
			$parameters['source'] = DataHolder::SOURCE_PARAMETERS;
		}
	 ...
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 */
	public function setParentContainer(Validator\IValidatorContainer $parent)
	{
	...
	protected function throwError($index = null, $affectedArgument = null, $argumentsRelative = false, $setAffected = false)
	{
		...
		if($this->hasParameter('translation_domain') && $this->getContext()->getTranslationManager()) {
			$error = $this->getContext()->getTranslationManager()->_($error, $this->getParameter('translation_domain'));
		}

		if(!$this->incident) {
			$this->incident = new Validator\ValidationIncident($this, self::mapErrorCode($this->getParameter('severity', 'error')));
		}

		foreach($affectedArguments as &$argument) {
			$argument = new Validator\ValidationArgument($argument, $this->getParameter('source'));
		}
		
		if($error !== null || count($affectedArguments) != 0) {
			// don't throw empty error messages without affected fields
			$this->incident->addError(new Validator\ValidationError($error, $index, $affectedArguments));
		}
	}
		...
		if($base->length() == 0) {
			...
			if($this->parentContainer !== null) {
				foreach($this->affectedArguments as $fieldname) {
					$this->parentContainer->addArgumentResult(new Validator\ValidationArgument($fieldname, $this->getParameter('source')), $result, $this);
				}
	 ...
	 */
	public function execute(DataHolder $parameters)
	{

		if($this->getParameter('source') != DataHolder::SOURCE_PARAMETERS && !in_array($this->getParameter('source'), $parameters->getSourceNames())) {
			throw new Configuration('Unknown source "' . $this->getParameter('source') . '" specified in validator ' . $this->getName());
		}
		$this->validationParameters = $parameters;

		// if we are given an array to match, explicitly set the params which match to the []
		foreach ($this->arguments as $arg):
			if (substr($arg, -2) == '[]'):
				foreach ($this->validationParameters->getSourceNames() as $sourceName):
					$params = $this->validationParameters->getAll($sourceName);
					$_arg = substr($arg, 0, strlen($arg) - 2);
					if (array_key_exists($_arg, $params)):
						foreach (array_keys($params[$_arg]) as $key):
							if ($sourceName == 'files'):
								if (isset($params[$_arg][$key]['name']) && strlen($params[$_arg][$key]['name']) > 0):
									$this->arguments = array_merge([$_arg . '[' . $key . ']'], $this->arguments);
								endif;
							else:
								$this->arguments = array_merge([$_arg . '[' . $key . ']'], $this->arguments);
							endif;
						endforeach;
						unset($this->arguments[array_search($_arg . '[]', $this->arguments)]);
					endif;
				endforeach;
			endif;
		endforeach;
	 ...
	 *
	 * @throws     Exceptions\Validator if the input was no known 
	 *                                           severity
	 *
	 * @author     Uwe Mesecke <uwe@mesecke.net>
	 */
	public static function mapErrorCode($code)
	{
		switch(strtolower($code)) {
			...
			case 'info':
				return self::INFO;
			default:
				throw new Validator('unknown error code: '.$code);
		}
	}
