...
use YoudsFramework\Translation\DateFormatter;
 ...
 *                 'calendar' (will return the Calendar object),
...
class DateTime extends Validator {
	...
	protected function validate()
	{
		if(!Config::get('core.use_translation')) {
			throw new Configuration('The datetime validator can only be used with use_translation on');
		}
		...
		if($this->hasMultipleArguments() && !$this->getParameter('arguments_format')) {
			...
			foreach($this->getArguments() as $calField => $field) {
				...
				if(defined($calField)) {
					...
					throw new Validator('Unknown argument name "' . $calField . '" for argument "' . $field . '" supplied. This needs to be one of the constants defined in DateDefinitions.');
				}
				if(!is_scalar($param)) {
	 ...
	 * @param      DateFormat The default format when parsing strings.
