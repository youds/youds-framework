...
use YoudsFramework\Request\Environment\Web;
...
/**
 * BaseFile is the base validator when validating files. 
 ...
 * @version    $Id$
 */
abstract class BaseFile extends Validator {
	/**
	...
	public function initialize(Context $context, array $parameters = array(), array $arguments = array(), array $errors = array())
	{
		if(!isset($parameters['source'])) {
			$parameters['source'] = Web::SOURCE_FILES;
		}

		parent::initialize($context, $parameters, $arguments, $errors);
		
		if($this->hasParameter('mime_type') && !extension_loaded('fileinfo')) {
			throw new Validator('MIME type checks in file validators require the "fileinfo" PHP extension to be loaded.');
		}
	}
