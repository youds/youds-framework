...
class ImageFile  extends BaseFile
{
	...
	protected function validate()
	{
		...
		if(!$this->hasParameter('format')) { 
			$this->export($file);
			return true; // because image format does not need validating
		}
		...
		if(!is_array($format)) {
			$format = explode(' ', $this->getParameter('format')); // accepts multiple parameters or as a string with spaces
		}
		$this->export($this->getArgument());
