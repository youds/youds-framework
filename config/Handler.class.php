...
/**
 * Handler allows a developer to create a custom formatted
 ...
 * @since      0.1
 *
 * @deprecated Superseded by XmlHandler, will be removed in Youds Framework 1.1
 *
 * @version    $Id$
 */
abstract class Handler extends BaseHandler implements ILegacyHandler {
	 ...
	 *
	 * @param      ValueHolder The node that contains a parameters child.
	 ...
	 *
	 * @see        XmlParser::testPattern()
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 */
	public static function testPattern($pattern, $subject)
	{
		return XmlParser::testPattern($pattern, $subject);
	}

	/**
	 * Returns a properly ordered array of ValueHolder configuration
	 * elements for given env and context.
	 *
	 * @param      ValueHolder The root config element
	 * @param      string                 An environment name.
	 * @param      string                 A context name.
	 * @param      bool                   Whether the parser class should be
	 *                                    autoloaded or not.
	 *
	 * @return     array An array of ValueHolder configuration elements.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 */
	public function orderConfigurations(ValueHolder $configurations, $environment = null, $context = null, $autoloadParser = true)
	{
		...
		if($configurations->hasAttribute('parent')) {
			...
			$parentConfigs = $this->orderConfigurations(Cache::parseConfig($parent, $autoloadParser, $this->getValidationFile(), $this->parser)->configurations, $environment, $context, $autoloadParser);
