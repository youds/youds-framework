...
/**
 * LdmlSupplementalHandler allows you to parse ldml supplemental data
 * file into an array.
 *
 ...
 * @version    $Id$
 */
class LdmlSupplementalHandler extends XmlHandler {
	/**
	...
	public function execute(XmlDomDocument $document)
	{
		...
		foreach($dataTree->getChild('currencyData') as $currencyNode) {
			if($currencyNode->localName == 'frchains') {
				foreach($currencyNode as $info) {
					$data['frchains'][$info->getAttribute('iso4217')] = array(
				...
				foreach($currencyNode as $currency) {
					if($currency->getName() == 'currency') {
						...
						throw new Exception('Invalid tag ' . $currency->localName . ' in region tag');
					}
				}
			} else {
				throw new Exception('Invalid tag ' . $currencyNode->localName . ' in currencyData tag');
			}
		}
		...
		foreach($dataTree->getChild('territoryContainment') as $group) {
			if($group->localName == 'group') {
				...
				throw new Exception('Invalid tag ' . $group->localName . ' in territoryContainment tag');
			}
		}
		...
		foreach($dataTree->getChild('languageData') as $language) {
			if($language->localName == 'language') {
				...
				throw new Exception('Invalid tag ' . $language->getName() . ' in languageData tag');
			}
		}
		...
		foreach($dataTree->getChild('weekData') as $entry) {
			...
			if($entryName == 'minDays') {
				...
				throw new Exception('Invalid tag ' . $entry->localName . ' in weekData tag');
			}
		...
		foreach($dataTree->getChild('timezoneData')->getChild('zoneFormatting') as $zoneItem) {
			if($zoneItem->localName == 'zoneItem') {
				...
				throw new Exception('Invalid tag ' . $language->localName . ' in zoneFormatting tag');
			}
		}
