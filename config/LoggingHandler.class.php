...
/**
 * LoggingHandler allows you to register loggers with the system.
 *
 ...
 * @version    $Id$
 */
class LoggingHandler extends XmlHandler {
	const XML_NAMESPACE = 'http://framework.youds.com/xml/config/parts/logging';
		...
		foreach($document->getConfigurationElements() as $cfg) {
			if($cfg->has('loggers')) {
				foreach($cfg->get('loggers') as $logger) {
					...
					$loggers[$name]['params'] = $logger->getParameters($loggers[$name]['params']);
				}
			}
			...
			if($cfg->has('appenders')) {
				foreach($cfg->get('appenders') as $appender) {
					...
					$appenders[$name]['params'] = $appender->getParameters($appenders[$name]['params']);
				}
			}
			...
			if($cfg->has('layouts')) {
				foreach($cfg->get('layouts') as $layout) {
					...
					$layouts[$name]['params'] = $layout->getParameters($layouts[$name]['params']);
				}
			}
			...
			if($cfg->has('loggers')) {
				...
				if(!isset($loggers[$defaultLogger])) {
					throw new Configuration(sprintf('Logger "%s" is configured as default, but does not exist.', $defaultLogger));
				}
			}
		}
		...
		if(count($loggers) > 0) {
			foreach($layouts as $name => $layout) {
				...
				if(!isset($layout['class'])) {
					throw new Configuration(sprintf('logging.xml has no class defined for layout "%s".', $name));
				} elseif(!class_exists('YoudsFramework\\' . $layout['class'])) {
					throw new Configuration(sprintf('logging.xml specifies unknown class "%s" for layout "%s".', $layout['class'], $name));
				}
			...
			foreach($appenders as $name => $appender) {
				if(!isset($appender['class'])) {
					throw new Configuration(sprintf('logging.xml has no class defined for appender "%s".', $name));
				} elseif(!class_exists('YoudsFramework\\' . $appender['class'])) {
					throw new Configuration(sprintf('logging.xml specifies unknown class "%s" for appender "%s".', $appender['class'], $name));
				}
				...
				if(!isset($appender['layout'])) {
					throw new Configuration(sprintf('logging.xml has no layout defined for appender "%s".', $name));
				} elseif(!isset($layouts[$appender['layout']])) {
					throw new Configuration(sprintf('logging.xml specifies unknown layout "%s" for appender "%s".', $appender['layout'], $name));
				}
			...
			foreach($loggers as $name => $logger) {
				if(!isset($logger['class'])) {
					throw new Configuration(sprintf('logging.xml has no class defined for logger "%s".', $name));
				} elseif(!class_exists('YoudsFramework\\' . $logger['class'])) {
					throw new Configuration(sprintf('logging.xml specifies unknown class "%s" for logger "%s".', $logger['class'], $name));
				}
				...
				foreach($logger['appenders'] as $appender) {
					if(!isset($appenders[$appender])) {
						throw new Configuration(sprintf('logging.xml specifies unknown appender "%s" for logger "%s".', $appender, $name));
					}
				...
				if($logger['level'] !== null) {
					$code[] = sprintf('${%s}->setLevel(%s);', var_export('_logger_' . $name, true), str_replace('Logger', 'Logging\Logger', $logger['level']));
				}
