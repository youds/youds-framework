...
/**
 * WsdlHandler simply writes the given WSDL file to disk.
 *
 ...
 * @version    $Id$
 */
class WsdlHandler extends XmlHandler {
	/**
	 ...
	 * @author     David Zülke <dz@bitxtender.com>
	 */
	public function execute(XmlDomDocument $doc)
	{
		$ro = $this->context->getRouting();
		
		$cleanName = preg_replace('/\W/', '', Config::get('core.project_name'));
		
		$xpath = new \DOMXPath($doc);
		$xpath->registerNamespace('soap', 'http://schemas.xmlsoap.org/wsdl/soap/');
		$xpath->registerNamespace('wsdl', 'http://schemas.xmlsoap.org/wsdl/');
		
		$paramWsdlDefinitionsName     = $ro->getParameter('wsdl_generator[wsdl][definitions][name]', $cleanName);
