...
use YoudsFramework\Date\DateDefinitions;
...
/**
 * LdmlHandler allows you to parse ldml files into an array.
 *
 ...
 * @version    $Id$
 */
class LdmlHandler extends Handler
{
	...
	public function execute($config, $context = null)
	{
		...
		foreach(array_reverse($lookupPaths) as $basename) {
			...
			if(is_readable($filePath)) {
				$ldmlTree = Cache::parseConfig($filePath, false, $this->getValidationFile(), $this->parser);
	 ...
	 *
	 * @param      ValueHolder The ldml tree.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 */
	protected function prepareParentInformation($ldmlTree)
	{
		$this->nodeRefs = array();
		$i = 0;
		$ldmlTree->setAttribute('__framework_node_id', $i);
		$ldmlTree->setAttribute('__framework_parent_id', null);
	 ...
	 *
	 * @param      ValueHolder The ldml node.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 */
	protected function generateParentInformation($childList, &$nextId, $parentId)
	{
		foreach($childList as $child) {
			$child->setAttribute('__framework_node_id', $nextId);
			$child->setAttribute('__framework_parent_id', $parentId);
			$this->nodeRefs[$nextId] = $child;
			++$nextId;
			if($child->hasChildren()) {
				$this->generateParentInformation($child->getChildren(), $nextId, $child->getAttribute('__framework_node_id'));
			}
		}
	}
...
*/
	...
	/**
	 * Generates the array used by Locale from an LDML tree.
	 *
	 * @param      ValueHolder The ldml tree.
							...
							foreach($dtfItems as $item) {
								if($item->getName() == 'dateTimeFormatLength') {
									if(isset($item->dateTimeFormat->pattern)) {
										...
										throw new Exception('unknown child content in dateTimeFormatLength tag');
									}
								} elseif($item->getName() == 'availableFormats') {
									foreach($item as $dateFormatItem) {
										if($dateFormatItem->getName() != 'dateFormatItem') {
											throw new Exception('unknown childtag "' . $dateFormatItem->getName() . '" in availableFormats tag');
										}
									...
									foreach($item as $appendItem) {
										if($appendItem->getName() != 'appendItem') {
											throw new Exception('unknown childtag "' . $appendItem->getName() . '" in appendItems tag');
										}
									...
									throw new Exception('unknown childtag "' . $item->getName() . '" in dateTimeFormats tag');
								}
							}
						}
	 ...
	 *
	 * @param      array List of ValueHolder items.
	 * @param      array The array to store the parsed data to.
	 *
	 * @return     array The array with the data.
	 *
	...
	protected function getTypeList($list, &$data)
	{
		...
		if($lc != count($list->getChildren())) {
			throw new Exception('wrong tagcount');
		}
	...
	protected function getChildsOrAlias($item)
	{
		if(isset($item->alias)) {
			...
			if($alias->getAttribute('source') != 'locale') {
				throw new Exception('The alias handling doesn\'t support any source except locale (' . $alias->getAttribute('source') . ' was given)');
			}

			$pathParts = explode('/', $alias->getAttribute('path'));

			$currentNodeId = $item->getAttribute('__framework_node_id');
			
			foreach($pathParts as $part) {
				// select the parent node
				if($part == '..') {
					$currentNodeId = $this->nodeRefs[$currentNodeId]->getAttribute('__framework_parent_id');
				} else {
					$predicates = array();
					if(preg_match('#([^\[]+)\[([^\]]+)\]#', $part, $match)) {
						if(!preg_match('#@([^=]+)=\'([^\']+)\'#', $match[2], $predMatch)) {
							throw new Exception('Unknown predicate ' . $match[2] . ' in alias xpath spec');
						}
						$tagName = $match[1];
					...
					foreach($this->nodeRefs[$currentNodeId]->getChildren() as $childNode) {
						...
						if($isSearchedNode) {
							$currentNodeId = $childNode->getAttribute('__framework_node_id');
						}
					}
				}
			}
