...
abstract class TimeZone
{
	...
	public function getDisplayName($daylight = null, $style = null, ?Locale $locale = null)
	{
		if($daylight === null) {
			...
			throw new InvalidArgument('Illegal arguments for TimeZone::getDisplayName');
		}
	 ...
	 * 
	 * @param      TimeZone The object to be compared with
	 * 
	...
	public static function createCustomTimeZone(TranslationManager $tm, $id)
	{
		...
		if(preg_match('#^GMT([+-])(\d{1,2}):(\d{1,2})(?::(\d{1,2}))?$#', $id, $match)) {
			...
			throw new InvalidArgument('Zone identifier is not parseable');
		}
