...
abstract class Calendar
{
	 ...
	 *
	 * @throws     Exceptions\Overflow when the date can't be represented by
	 *                                      an unix timestamp.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 */
	public function getUnixTimestamp()
	{
		...
		if($unixTime != $unixTimeInt) {
			throw new Overflow('cannot convert the date ' . $this->get(DateDefinitions::YEAR) . '/' . $this->get(DateDefinitions::MONTH) . '/' . $this->get(DateDefinitions::DATE) . ' into a unix timestamp');
		}
		return $unixTimeInt;
	}
	 ...
	 *
	 * @param      array An array using the DateDefinitions::XXX as key 
	...
	public function add($field, $amount)
	{
		...
		switch($field) {
			...
			default:
				throw new InvalidArgument('Calendar::add(): field ' . $field . ' is not supported');
				return;
		}
	...
	public function roll($field, $amount)
	{
		...
		default:
				...
				throw new InvalidArgument('Calendar::roll(): field ' . $field . ' cannot be rolled with this method');
		}
	}
	 ...
	 *
	 * @param      float|Calendar     When the date to compare this 
	 *                                     calendar's time to
			...
			while(true) {
				...
				if($ms == $targetMs) {
					...
					if($max < 0) {
						...
						throw new InvalidArgument('The difference is to large to fit into an integer');
					}
				}
			}
			// Do a binary search
	 ...
	 *
	 * @param      TimeZone The given time zone. If null is passed nothing
	 *                           is done.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
...
//linesInGroup:
	...
	protected function weekNumber1($desiredDay, $dayOfPeriod, $dayOfWeek)
	{
		...
		// may be frchainal (or may not be).  We add periodStartDayOfWeek in
		// order to fill out the first week, if it is frchainal.
		...
		switch($field) {
			...
			case DateDefinitions::DAY_OF_WEEK_IN_MONTH:
				if($this->internalGet($field) == 0) {
					throw new InvalidArgument('DAY_OF_WEEK_IN_MONTH cannot be zero');
					return;
				}
	 ...
	 * <code>Exceptions\InvalidArgument</code> will be thrown.  Subclasses may
	...
	private function validateField1($field, $min, $max)
	{
		...
		if($value < $min || $value > $max) {
			throw new InvalidArgument('Illegal argument error. Field: ' . $field . '. Value ' . $value . ' is not within ' . $min . ' and ' . $max);
		}
	}
