...
class DateFormat
{
	 ...
	 *                   needed info with the DateDefinitions constants as 
	 ...
	 * @param      Locale|string The locale to format the date in.
	 *
	...
	public function format($data, $calendarType = Calendar::GREGORIAN, $locale = null)
	{
		if($locale instanceof Locale) {
			...
			throw new InvalidArgument('This DateFormat has not been initialize()d. To be able to pass a string as locale you need to call initialize() or create this DateFormat using TranslationManager::createDateFormat()');
		}
				...
				if($c == '\'') {
					...
					if(preg_match('#[a-z]#i', $c)) {
						if(isset($charToTokenMap[$c])) {
							...
							throw new Exception('Unknown pattern char "' . $c . '" (#'. ord($c) . ') in format string "' . $format . '" at index ' . $i);
						}
					} else {
						$this->tokenList[$tokenIdx] = array(self::T_TEXT, $c);
						++$tokenIdx;
	...
	public function parse($dateString, $locale = null, $strict = false)
	{
		...
		for($i = 0; $i < $tlCount; ++$i) {
			if($datePos >= strlen($dateString)) {
				if($strict) {
					throw new Exception('Input string "' . $dateString . '" is to short');
				}
				break;
			}
			...
			if($type == 'number' && $i + 1 < $tlCount && $this->getTokenType($this->tokenList[$i + 1]) == 'number') {
				...
				if($diff > 0) {
					...
					if($diff >= $tLen - 1) {
						...
						throw new Exception('Not enough digits given in "' . $dateString . '" at pos ' . $datePos . ' (expected ' . $tokenReqLen . ' digits)');
					}
				}
				...
				foreach($unprocessedTokens as $token) {
					...
					if($dateField === null) {
						throw new Exception('Token type ' . $token[0] . ' claims to be numerical but has no date field');
					}
					...
					if($dateField == DateDefinitions::HOUR_OF_DAY && $token[0] == DateFormat::T_HOUR_1_24 && $number == 24) {
						$number = 0;
					} elseif($dateField == DateDefinitions::HOUR && $token[0] == DateFormat::T_HOUR2 && $number == 12) {
						$number = 0;
				...
				if($dateField == DateDefinitions::MILLISECOND && $numberLen > 3) {
					// we only store in millisecond precision so only use the first 3 digits of the frchainal second
				...
				switch($token[0]) {
					case self::T_TEXT:
						if(substr_compare($dateString, $token[1], $datePos, strlen($token[1])) == 0) {
							...
							throw new Exception('Unknown character in "' . $dateString . '" at pos ' . $datePos . ' (expected: "' . $token[1] . '", got: "' . substr($dateString, $datePos, strlen($token[1])) . '")');
						}
						break;
					...
					case self::T_SA_LOCAL_DAY_OF_WEEK:
						...
						if($this->matchStringWithFallbacks($dateString, $items, $datePos, $item)) {
							...
							throw new Exception('Unknown character in "' . $dateString . '" at pos ' . $datePos . ' (expected one of: ' . implode(', ', $items) . ')');
						}
						break;
						...
						if(preg_match('#^(GMT)?(\+|-)?(\d{1,2}:\d{1,2}|\d{1,2}\d{1,2})#i', $remainder, $match)) {
							...
							if(!($tz = $tm->createTimeZone($remainder))) {
								...
								if($this->matchStringWithFallbacks($dateString, $localizedTzMap, $datePos, $id)) {
									...
									throw new Exception('Unknown character in "' . $dateString . '" at pos ' . $datePos . ' (expected one of: ' . implode(', ', $localizedTzMap) . ')');
								}
							} else {
					...
					case self::T_SA_MONTH:
						...
						if($this->matchStringWithFallbacks($dateString, $months, $datePos, $month)) {
							...
							throw new Exception('Unknown character in "' . $dateString . '" at pos ' . $datePos . ' (expected one of: ' . implode(', ', $months) . ')');
						}
					...
					case self::T_SA_QUARTER:
						...
						if($this->matchStringWithFallbacks($dateString, $quarters, $datePos, $quarter)) {
							...
							throw new Exception('Unknown character in "' . $dateString . '" at pos ' . $datePos . ' (expected one of: ' . implode(', ', $quarters) . ')');
						}
						break;
		...
		if($strict) {
			...
			if($datePos < strlen($dateString)) {
				throw new Exception('Input string "' . $dateString . '" has characters after the date');
			}
		}
	...
	protected function getDateFieldFromTokenType($type)
	{
		static $typeMap = array(
			...
			self::T_HOUR2               => DateDefinitions::HOUR,
