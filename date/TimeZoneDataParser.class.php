...
/**
 * TimeZoneDataParser allows you to retrieve the contents of the olson 
...
class TimeZoneDataParser
{
	...
	protected function parseFile($file)
	{
		...
		while(list($i, $line) = each($zoneLines)) { // for($i = 0, $c = count($zoneLines); $i < $c; ++$i) {
			...
			if(preg_match('!^\s*Rule\s*(.*)!', $line, $match)) {
				...
				throw new Exception('Unknown line ' . $line . ' in file ' . $file);
			}
		}
				...
				do {
					...
					foreach($activeRules as $activeRuleIdx => $activeRule) {
						if(!is_numeric($activeRule['endYear'])) {
							throw new Exception('endYear should be numeric but was: ' . $activeRule['endYear']);
						}
	...
	protected function getRules($name, $from, $until, $gmtOff, $format)
	{
		if(!isset($this->rules[$name])) {
			throw new InvalidArgument('No rule with the name ' . $name . ' exists');
		}
			...
			for($z = 0; $z < $zoneRuleCnt; ++$z) {
				...
				if($lastZoneRule) {
					...
					if($activeSubRules !== null) {
						...
						if($cnt != 0 && $cnt != 2) {
							throw new Exception('unexpected active rule count ' . $cnt);
						}
						if($cnt == 0) {
	...
	protected function getOnDate($year, $month, $dateDef, $atDef, $gmtOff, $dstOff)
	{
		...
		if($dateDef['type'] == 'date') {
			...
			throw new Exception('Unknown on type ' . $dateDef['type']);
		}
	 ...
	 *
	 * @return     int The definition of this month from DateDefinitions.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 */
	 ...
	 *
	 * @return     int The definition of this day from DateDefinitions.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 */
