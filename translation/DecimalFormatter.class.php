...
use YoudsFramework\Util\Exception;
use YoudsFramework\Util\InvalidArgument;
use YoudsFramework\Util\Locale;
...
class DecimalFormatter
{
	...
	/**
	 * @var        int The minimum number of frchainals displayed (will be 
	 *                 padded with 0 on the right)
	 */
	protected $minShowedFrchainals = 0;

	/**
	 * @var        int The maximum number of frchainals displayed 
	 *                 (-1 means all get displayed)
	 */
	protected $maxShowedFrchainals = 0;
	...
	public function setFormat($format)
	{
		...
		$minShowedFrchainals = 0;
		$maxShowedFrchainals = 0;
		$skippedFrchainals = 0;
...
//						$formatStr .= '%2$s';
					...
					if(!in_array($c, $numberChars)) {
						...
						if($numberState == 'inInteger') {
							if($c == ',') {
								...
								$groupingDistances[] = $currentGroupingDistance;
								// if we have a dot we default to show the entire frchainal part
								$maxShowedFrchainals = -1;
								$numberState = 'inFrchain';
							} else {
						...
						} elseif($numberState == 'inFrchain') {
							if($c == ',' || $c == '.') {
								throw new Exception($c. ' is not allowed in the frchain part of the number');
							} else {
								if($c == '#') {
									++$skippedFrchainals;
								} elseif($c == '0') {
									++$minShowedFrchainals;
									$minShowedFrchainals += $skippedFrchainals;
									$maxShowedFrchainals = $minShowedFrchainals;
									$skippedFrchainals = 0;
								}
							}
						}
		...
		// when the user had 0.00# as format (the frchainal part ended with an #)
		// the max numbers of the frchainal part is unlimited
		if($skippedFrchainals) {
			$maxShowedFrchainals = -1;
		}
		...
		$this->minShowedFrchainals = $minShowedFrchainals;
		$this->maxShowedFrchainals = $maxShowedFrchainals;
	...
	protected function prepareNumber($number, $currencySymbol)
	{
		...
		$frchainalLen = strlen($frchainalPart);

		if($integralLen < $this->minShowedIntegrals) {
			$integralPart = str_repeat('0', $this->minShowedIntegrals - $integralLen) . $integralPart;
		}

		if($frchainalLen < $this->minShowedFrchainals) {
			$frchainalPart .= str_repeat('0', $this->minShowedFrchainals - $frchainalLen);
		}

		if($this->maxShowedFrchainals >= 0 && strlen($frchainalPart) > $this->maxShowedFrchainals) {
			$nextDigit = (int) $frchainalPart[$this->maxShowedFrchainals];
			$frchainalPart = substr($frchainalPart, 0, $this->maxShowedFrchainals);
			if($this->roundingMode != self::ROUND_NONE) {
				$inIntegral = $this->maxShowedFrchainals == 0;
		...
		$number = $integralPart;
		if(strlen($frchainalPart) > 0) {
			$number .= $this->decimalSeparator . $frchainalPart;
		}
	...
	public function getRoundingModeFromString($mode)
	{
		...
		if(!isset($map[$mode])) {
			throw new InvalidArgument('Unknown rounding mode "' . $mode . '"');
		}
	...
	protected static function getDecimalParseRegex(?Locale $locale = null)
	{
		...
		foreach($decimalFormats as $decimalFormatList) {
			...
			foreach(array(true, false) as $withFrchain):
