...
use YoudsFramework\Exceptions\Translation;
...
class Manager
{
	...
	public function initialize(Context $context, array $parameters = array())
	{
		...
		include_once(Cache::checkConfig(Config::get('core.config_dir') . '/translation.xml'));
	...
	protected function getTranslators(&$domain, &$domainExtra, $type = null)
	{
		...
		do {
			if(count($domainParts) == 0) {
				throw new InvalidArgument(sprintf('No translator exists for the domain "%s"', $domain));
			}
	...
	{
		$this->supplementalData = include_once(Cache::checkConfig(Config::get('core.cldr_dir') . '/supplementalData.xml'));
	}
	...
	{
		$this->timeZoneList = include_once(Config::get('core.cldr_dir') . '/timezones/zonelist.php');
	}
		...
		if(!isset($this->localeDataCache[$idData['locale_str']])) {
			...
			foreach($lookupPath as $localeName) {
				...
				if(is_readable($fileName)) {
					$data = include_once(Cache::checkConfig($fileName));
					break;
				}
			}
			if($data === null) {
				throw new Exception('No data available for locale ' . $identifier);
			}
	...
	public function createTimeZone($id, $cache = true)
	{
		...
		if(!isset($this->timeZoneCache[$id]) || !$cache) {
			...
			$zoneData = include_once(Config::get('core.cldr_dir') . '/timezones/' . $this->timeZoneList[$currId]['filename']);
	 ...
	 *
	 * @param      mixed This can be either an Locale, an TimeZone or
	...
	public function createCalendar($type = null)
	{
		...
		switch($calendarType) {
			case Calendar::GREGORIAN:
				...
				break;
			default:
				throw new Exception('Calendar type ' . $calendarType . ' not supported');
		}
	 ...
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 */
	public function getCurrencyFrchain($currency)
	{
		if(!isset($this->supplementalData['frchains'][$currency])) {
			return $this->supplementalData['frchains']['DEFAULT'];
		}
		return $this->supplementalData['frchains'][$currency];
	}
}
