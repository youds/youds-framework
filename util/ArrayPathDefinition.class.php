	...
	public static function getPartsFromPath($path)
	{
		...
		if(($pos = strpos($path, '[')) === false) {
			if(strpos($path, ']') !== false) {
				throw new InvalidArgument('Invalid "]" without opening "[" found');
			}
			$parts[] = $path;
		} else {
			$state = 0;
				...
				switch($state) {
					...
					case 2:
						...
						if($c == ']') {
							...
							throw new InvalidArgument('Invalid "[[" found');
						} else {
							$cur .= $c;
						}
					...
					case 1:
						...
						if($c == '[') {
							...
							throw new InvalidArgument('Invalid character after "]" found');
						}
						break;
			...
			if($state == 0) {
				...
				throw new InvalidArgument('Missing "]" after opening "["');
			}
		}
