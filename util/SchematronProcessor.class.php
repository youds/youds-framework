...
use YoudsFramework\Config\XsltProcessor;
...
class SchematronProcessor extends ParameterHolder
{
	...
	protected static $defaultChain = array(
		'%core.src_dir%/config/schematron/iso_dsdl_include.xsl',
		'%core.src_dir%/config/schematron/iso_abstract_expand.xsl',
		'%core.src_dir%/config/schematron/iso_svrl_for_xslt1.xsl'
	);
	...
	/**
	 * @var 		array The chain for execution
	 */
	protected $chain;
	...
	public function __construct(?array $chain = null)
	{
		...
		$this->chain = array_map(array('YoudsFramework\Util\Toolkit', 'expandDirectives'), $chain);
	}
	...
	public function transform(\DOMDocument $schema)
	{
		...
		if($this->node === null) {
			throw new Parse('Schema validation failed because no document could be parsed');
		}
		
		// is it an ISO Schematron file?
		if(!$schema->documentElement || $schema->documentElement->namespaceURI != self::NAMESPACE_SCHEMATRON_ISO) {
			throw new Parse(sprintf('Schema file "%s" is invalid', $schema->documentURI));
		}
		...
		foreach($this->getProcessors() as $processor) {
			...
			try {
				...
				throw new Parse(sprintf('Could not transform schema file "%s": %s', $schema->documentURI, $e->getMessage()), 0, $e);
			}
			if($first) {
				$this->cleanupProcessor($processor);
				$first = false;
		...
		if(!$validatorImpl->documentElement || $validatorImpl->documentElement->namespaceURI != self::NAMESPACE_XSL_1999) {
			throw new Parse(sprintf('Processing using schema file "%s" resulted in an invalid stylesheet', $schema->documentURI));
		}
		...
		try {
			...
			throw new Parse(sprintf('Could not process the schema file "%s": %s', $schema->documentURI, $e->getMessage()), 0, $e);
		}
		...
		try {
			...
			throw new Parse(sprintf('Could not validate the document against the schema file "%s": %s', $schema->documentURI, $e->getMessage()), 0, $e);
		}
