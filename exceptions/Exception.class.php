...
/**
 * Exception is the base class for all Youds Framework related exceptions and
 * provides an additional method for printing up a detailed layout of an
 * exception.
 *
...
class Exception extends \Exception
{
	 ...
	 * @author     David Zülke <dz@bitxtender.com>
	 *
	 * @deprecated Superseded by Exception::render()
	 */
	 ...
	 * @author     David Zülke <david.zuelke@bitextender.com>
	 */
	public static function highlightString($code, $theme = 'light')
	{
		...
		// convert color to classes for dark mode
		$code = str_replace('style="color: #0000BB"', 'class="primary"', $code);
		$code = str_replace('style="color: #007700"', 'class="constructors"', $code);
		$code = str_replace('style="color: #FF8000"', 'class="comments"', $code);
	...
	public static function render(\Exception $e, ?Context $context = null, ?ExecutionContainer $container = null)
	{
		...
		if($container !== null && $container->getOutputType() !== null && $container->getOutputType()->getTemplate() !== null) {
			// an exception template was defined for the container's output type
			include_once($container->getOutputType()->getTemplate()); 
			exit($exitCode);
		}
		...
		if($context !== null && $context->getController() !== null) {
			try {
				...
				if($context->getController()->getOutputType()->getTemplate() !== null) {
					include_once($context->getController()->getOutputType()->getTemplate());
					exit($exitCode);
				}
			} catch(Exception $e2) {
				unset($e2);
		...
		if($context !== null && Config::get('exception.templates.' . $context->getName()) !== null) {
			...
			include_once(Config::get('exception.templates.' . $context->getName()));
			exit($exitCode);
		}
		
		// include default exception template
		include_once(Config::get('exception.default_template'));
