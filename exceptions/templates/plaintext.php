...
if(!defined('STDOUT') || (function_exists('posix_isatty') && !posix_isatty(STDOUT))) {
	...
	$cols = false;
} elseif(function_exists('exec') && (file_exists('/bin/stty') && is_executable('/bin/stty') && $sttySize = exec('/bin/stty size 2>/dev/null'))) {
...
<?php
$i = 0;
$traceLines = Exception::getFixedTrace($e, isset($exceptions[$ei+1]) ? $exceptions[$ei+1] : null);
...
=======================
  ...
  :     <?php echo $cols ? wordwrap(Config::get('framework.version'), $cols - 13, "\n             ", true) : Config::get('framework.version'); ?>
