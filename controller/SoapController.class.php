...
{
	/**
	 * @param      DataHolder Additional request data for later use.
	 */
	...
	public function startup()
	{
		...
		// get the name of the class to use for handling soap calls, defaults to 's "SoapControllerCallHandler"
		...
		if($this->getParameter('auto_headers', true)) {
			...
			$cache = Cache::getCacheName($soapHandlerClass, $this->context->getName());
			
			if(Cache::isModified($wsdl, $cache)) {
				$doc = new \DOMDocument();
				$doc->load($wsdl);
				...
				Cache::writeCacheFile($soapHandlerClass, $cache, $code);
			}
