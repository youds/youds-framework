...
use YoudsFramework\Request\UploadedFile;
// +----------------------------------------------------------------------------+
...
/**
 * Web provides additional support for web-only client requests
 ...
 * @version    $Id$
 */
class Web extends Base implements RequestBase
{
	/**
	...
	/**
	 * @var        string The current request source.
	 */
	const SOURCE_FILES = 'files';

	/**
	 * Get the request protocol information, e.g. "HTTP".
	 *
	...
	public function __construct()
	{
		...
		$this->setParameters(array(
			'request_data_holder_class' => 'Request\Web',
		));
	}
	...
	{
		// this method only works with PHP 0.1+
		// there used to be special code for versions < 0.1
		// http://bugs.php.net/bug.php?id=41093
		// but we now require 0.1 anyway in combination with magic_quotes_gpc, see initialize()
	...
	public function initialize(Context $context, array $parameters = array())
	{
		...
		// check for ajax
		$this->setParameter('ajax', ((isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] !== NULL) || (isset($_SERVER['HTTP_SEC_FETCH_MODE']) && $_SERVER['HTTP_SEC_FETCH_MODE'] == 'cors'))?true:false);
		...
		// the result is an XSS attack vector, e.g. on Web::gen(null)
		...
		$this->setRequestData(new $rdhc(array(
			constant("YoudsFramework\Request\DataHolder::SOURCE_PARAMETERS") => array_merge($_GET, $_POST),
			constant("YoudsFramework\Request\Web::SOURCE_COOKIES") => $_COOKIE,
			constant("YoudsFramework\Request\Web::SOURCE_FILES") => $files,
			constant("YoudsFramework\Request\Web::SOURCE_HEADERS") => $headers,
		)));
    }

	/**
	 * Corrects the order of $_FILES for arrays of files.
	 * The cleaned up array of UploadedFile objects is returned.
	 *
