...
/**
 * UploadedFile is a container with information for files that were
...
class UploadedFile implements ArrayAccess
{
	...
	public function __construct(array $array)
	{
		...
		if(
			...
			throw new InvalidArgument('Value for exactly one of keys "tmp_name", "contents" or "stream" must be supplied.');
		}
	...
	{
		trigger_error('Property access in UploadedFile is deprecated and may be removed in a later version of Youds Framework. Please use getter methods or array access instead.', E_USER_DEPRECATED);
	 ...
	 *
	 * @throws     Exceptions\BadMethodCall UploadedFile objects are immutable.
	 *
	 * @author     David Zülke <david.zuelke@bitextender.com>
	 */
	public function offsetSet($key, $value): void
	{
		throw new BadMethodCall('UploadedFile objects are immutable.');
	}
	...
	public function getTmpName()
	{
		if(!$this->hasTmpName() && !$this->hasOpenStream()) {
			...
			$this->stream = tmpfile();
			if(!$this->stream) {
				throw new File('Cannot create temporary file via tmpfile()');
			}
	 ...
	 * or another type of file submission handled by .
	 *
	 ...
	 *
	 * @throws     Exceptions\File If chmod or mkdir calls failed.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 */
	public function move($dest, $fileMode = 0664, $create = true, $dirMode = 0775)
	{
		...
		if($moved) {
			...
			if(!@chmod($dest, $fileMode)) {
				throw new File('Failed to chmod uploaded file after moving');
			}
		} else {
			// moving the file failed
			throw new File('Failed to move uploaded file');
		}
