...
use YoudsFramework\Exceptions\ExceptionsFilter;
...
use YoudsFramework\Logging\Logger;
 ...
 * which usually happens when a Layout::INPUT is returned again after a POST
 ...
 * @version    $Id$
 */
class FormPopulation extends Filter implements IGlobal
{
	...
	public function execute(Chain $filterChain, ExecutionContainer $container)
	{
		...
		$cfg = $rq->getAttributes('org.framework.filter.FormPopulationFilter');
		...
		if(libxml_get_last_error() !== false) {
			...
			if(Config::get('core.use_logging') && $cfg['log_parse_errors'] !== false && $maxError >= $cfg['log_parse_errors']) {
				...
				$mc = 'YoudsFramework\Logging\\' . $lm->getDefaultMessageClass();
		...
		if(!$utf8 && $encoding != self::ENCODING_ISO_8859_1 && !function_exists('iconv')) {
			throw new Exception('No iconv content available, input encoding "' . $encoding . '" cannot be handled.');
		}
					...
					foreach($vr->byArgument($argument)->getIncidents() as $incident) {
						if(($incidentKey = array_search($incident, $allIncidents, true)) !== false) {
							...
							foreach($incidentArguments as $incidentArgument) {
								if(in_array($incidentArgument->getSource(), array(Web::SOURCE_FILES, DataHolder::SOURCE_PARAMETERS))) {
		...
		$rq->setAttribute('orphaned_errors', $allIncidents, 'org.framework.filter.FormPopulationFilter');
			...
			if($firstError) {
				...
				switch($firstError) {
					...
					case PREG_RECURSION_LIMIT_ERROR:
						$error .= "the recursion limit (defined by \"pcre.recursion_limit\") was hit. This shouldn't happen unless you changed that limit yourself in php.ini or using ini_set(). If the problem is not on your end, please file a bug report with a reproduce case on the Youds Framework issue tracker or drop by on the IRC support channel.";
						break;
					...
					default:
						$error .= "an internal PCRE error occurred. As a quick countermeasure, try to upgrade PHP (and the bundled PCRE) as well as libxml (yes!) to the latest versions to see if the problem goes away. If the issue persists, file a bug report with a reproduce case on the Youds Framework issue tracker or drop by on the IRC support channel.";
				}
	...
	protected function insertErrorMessages(\DOMElement $element, array $rules, array $incidents)
	{
		...
		foreach($rules as $xpathExpression => $errorMessageInfo) {
			...
			if(!$errorMarkup && !$errorContainer) {
				throw new Exception('Form Population Filter was unable to insert error messages into the document using the XPath expression "' . $xpathExpression . '" because the element information did not contain either a "markup" or "container" entry to use.');
			}
			...
			if($errorMarkup) {
				foreach($errors as $error) {
					if(is_callable($errorMarkup)) {
						...
						throw new Exception('Form Population Filter was unable to insert an error message into the document using the XPath expression "' . $xpathExpression . '" because the element information could not be evaluated as an XML/HTML fragment or as a PHP callback.');
					}
			...
			if($errorContainer) {
				...
				if(is_callable($errorContainer)) {
					...
					throw new Exception('Form Population Filter was unable to insert an error message container into the document using the XPath expression "' . $xpathExpression . '" because the element information could not be evaluated as an XML/HTML fragment or as a PHP callback.');
				}
	...
	public function initialize(Context $context, array $parameters = array())
	{
		...
		$this->context->getRequest()->setAttributes($this->getParameters(), 'org.framework.filter.FormPopulationFilter');
	}
}
