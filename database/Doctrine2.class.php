 ...
 * @version    $Id$
 */
abstract class Doctrine2 extends Database implements DatabaseInterface
{
	/**
	...
	public function initialize(Manager $databaseManager, array $parameters = array())
	{
		...
		if(!class_exists('Doctrine\Common\ClassLoader')) {
			// no soup for you!
			require_once('Doctrine/Common/ClassLoader.php'); // let's assume Doctrine2 is on ze include path...
		}
		...
		foreach((array)$this->getParameter('class_loaders', array('Doctrine' => null)) as $namespace => $includePath) {
			if($namespace == 'Doctrine' && class_exists('Doctrine\ORM\Version')) {
				// the ORM namespace's Version class exists or could be autoloaded; let's assume that the class loader for any Doctrine stuff won't need registration then
				continue;
			}
