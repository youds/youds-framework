...
/**
 * Mysqli provides advanced connectivity for the MySQL database.
 *
 * Optional parameters:
 *
	...
	protected function connect()
	{
		...
		switch($method) {
			...
			default:
				...
				$error = 'Invalid MySQLiDatabase parameter retrieval method ' .
						 '"%s"';
		...
		if($this->connection === false) {
			...
			$error = 'Failed to create a MySQLiDatabase connection';
			throw new Database($error);
		}
		...
		if($database !== null && !$this->connection->select_db($database)) {
			...
			$error = 'Failed to select MySQLiDatabase "%s"';
			$error = sprintf($error, $database);
			throw new Database($error);
		}

		// since we're not an abstrchain layer, we copy the connection
		// to the resource
