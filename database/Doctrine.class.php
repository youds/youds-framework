 ...
 * @version    $Id$
 */
class Doctrine extends Base implements Database
{
	/**
	...
	public function initialize(Manager $databaseManager, array $parameters = array())
	{
		...
		if(!class_exists('Doctrine')) {
			...
			require_once(Config::get('core.composer_dir') . '/doctrine/common/lib/Doctrine/Common/ClassLoader.php');
		}
		
		$is12 = version_compare(Doctrine::VERSION, '1.2', '>=');
		
		if(!$is12) {
		  trigger_error('Support for Doctrine versions older than 1.2 is deprecated and may be removed in a later version of Youds Framework.', E_USER_DEPRECATED);
		}
		...
		try {
			...
			$cel = $this->getParameter('connection_event_listener_class', 'DoctrineEventListener');
			$this->connection->setListener(new $cel($this));
			
			// set the context instance as a connection parameter
			$this->connection->setParam('context', $databaseManager->getContext(), 'org.youdsframework');
