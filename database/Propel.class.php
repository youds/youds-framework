...
/**
 * An Youds Framework Database driver for Propel. Supports Propel 1.3 and later.
 * 
 * Optional parameters:
 *
 * # config         - [none]    - path to the Propel runtime config file
 * # datasource     - [default] - datasource to use for the connection
 * # use_as_default - [false]   - use as default if multiple connections
 ...
 *                                       be used.
 * # enable_instance_pooling - [none] - set this to false if you want to 
 ...
 * @version    $Id$
 */
class Propel extends Base implements Database
{
	/**
	...
	public function initialize(Manager $databaseManager, array $parameters = array())
	{
		...
		if($datasource === null || $datasource == 'default') {
			if(isset($config['propel']['datasources']['default'])) {
				...
				throw new Database('No datasource given for Propel connection, and no default datasource specified in runtime configuration file.');
			}
		}
		...
		if($is13) {
			// old-style config array; PropelConfiguration was added after 0.1, http://trac.agavi.org/ticket/1195
