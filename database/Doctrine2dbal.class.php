 ...
 * its own, or in conjunction with Doctrine2orm (which in turn does
 * not require a separate Doctrine2dbal).
 *
 ...
 * @version    $Id$
 */
class Doctrine2dbal extends Doctrine2
{
	/**
	...
	public function connect()
	{
		...
		// boom
		try {
			$this->connection = \Doctrine\DBAL\DriverManager::getConnection((array)$this->getParameters(), $config, $eventManager);
		} catch(Exception $e) {
			throw new Database(sprintf("Failed to create connection '%s':\n\n%s" . $this->getName(), $e->getMessage()), 0, $e);
		}
	}
}
