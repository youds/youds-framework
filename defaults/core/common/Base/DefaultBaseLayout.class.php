...
use YoudsFramework\Layout as Base;
use YoudsFramework\ExecutionContainer;
use YoudsFramework\Exceptions\Layout as LayoutException;

class Layout extends Base
{
	/*
		This is the base layout all your application's layouts should extend.
		This way, you can easily inject new functionality into all of your layouts.
		...
		was declared in your layout, and no such method exists in this class either.
	...
	*/
	...
	public function initialize(ExecutionContainer $container)
	{
		...
		$this->it = $this->getContext()->getIntegrations();
		$this->gt = $this->getContext()->getGenerator();
	}
	
	public final function execute($rd)
	{
		throw new LayoutException(sprintf(
			'The Layout "%1$s" does not implement an "execute%3$s()" method to serve '.
			'the Output Type "%2$s", and the base Layout "%4$s" does not implement an '.
	...
	public function executeHtml($rd)
	{
		throw new LayoutException(sprintf(
			...
			'the method "execute%3$s()" in the base Layout "%4$s" that is throwing '.
			'this exception to deal with this situation in a more appropriate '.
			'way, for example by forwarding to the default 404 error chain, or by '.
			'showing some other meaningful error message to the user which explains '.
			'that the operation was unsuccessful because the desired Output Type is '.
			'not implemented.',
			get_class($this),
			$this->container->getOutputType()->getName(),
			ucfirst(strtolower($this->container->getOutputType()->getName())),
	...
	public function setupHtml($rd, $layoutName = null)
	{
